      ...
      <div style="padding: 32px; border: 1px solid #F0F0F0; border-top: none; border-radius: 0 0 16px 16px;">
        ...
        <div style="background: #FFF5F5; border-radius: 10px; padding: 14px 16px; margin-bottom: 24px;">
          <p style="color: #E53E3E; font-size: 12px; margin: 0; line-height: 1.6;">
            💳 Η επιστροφή <strong>€${data.total}</strong> θα γίνει εντός <strong>5-7 εργάσιμων ημερών</strong> στην κάρτα που χρησιμοποιήθηκε.
          </p>
        </div>
  ...
  try {
    ...
    if (booking.stripe_payment_intent_id) {
      try {
        ...
        console.log('Stripe refund created for:', booking.booking_ref)
      } catch (stripeErr: any) {
        console.error('Stripe refund error:', stripeErr.message)
        // Continue — booking is already cancelled, refund can be done manually
      }
    }
      ...
      try {
        await resend.emails.send({
          ...
          html: cancellationEmailHtml({
            ...
            total: Number(booking.total_amount || 0).toFixed(0),
          }),
        })
