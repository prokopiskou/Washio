...
type Booking = {
  ...
  locations: { name: string } | null; services: { name: string } | null
}
...
export default function ProfilePage() {
  ...
  const [showCarEdit, setShowCarEdit] = useState(true)
  ...
  useEffect(() => {
    const loadUser = async () => {
      ...
      if (!user) { router.push('/login'); setAuthLoading(false); return }
  ...
  const handleSaveProfile = async () => {
    ...
    const supabase = createClient()
    const { error } = await supabase.auth.updateUser({ data: { full_name: fullName.trim(), phone: phone.trim() } })
    setProfileSaving(false)
    if (!error) { setSavedProfile(true); setTimeout(() => setSavedProfile(false), 2000); setShowProfileEdit(false) }
  }
  ...
  const handleSaveVehicle = async () => {
    ...
    const supabase = createClient()
    const { error } = await supabase.from('vehicles').insert({ user_id: userId, plate: plate.trim().toUpperCase(), type: vehicleType })
    setVehicleSaving(false)
    if (!error) {
      await loadVehicles(userId)
      setPlate(''); setVehicleType('ΙΧ'); setSavedCar(true)
      setTimeout(() => setSavedCar(false), 2000); setShowVehicleForm(false); successHaptic()
    }
  }
        ...
        <div className="px-5 pt-4 flex flex-col gap-3.5">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.03)' }}>
            ...
            <div className="px-2 pb-2">
              {favorites.length === 0 ? (
                <p className="text-xs text-gray-400 px-3 py-2">{t.noFavorites}</p>
              ) : (
                favorites.map((fav, i) => (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.03)' }}>
            ...
            <div className="px-2 pb-2">
              {bookings.length === 0 ? (
                <p className="text-xs text-gray-400 px-3 py-2">{t.noBookings}</p>
              ) : (
                bookings.map((b, i) => (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.03)' }}>
            ...
            {showProfileEdit && (
              <div className="px-[18px] pb-4 pt-1 flex flex-col gap-2">
                <input type="text" value={fullName} onChange={e => setFullName(e.target.value)} placeholder={t.name}
                ...
                <button onClick={handleSaveProfile} disabled={profileSaving || savedProfile}
                  ...
                  {savedProfile ? (<span className="inline-flex items-center gap-1.5"><CheckCircle size={14} className="text-green-400" />{t.saved}</span>) : profileSaving ? t.saving : t.save}
                </button>
              </div>
            )}
          </div>
              ...
              <div className="px-[18px] pb-4 pt-1 flex flex-col gap-2">
                ...
                {showVehicleForm ? (
                  <div className="flex flex-col gap-2 pt-1">
                    <input type="text" value={plate} onChange={e => setPlate(e.target.value.toUpperCase())} placeholder={t.plate}
                    ...
                    <div className="flex gap-2">
                      <button onClick={handleSaveVehicle} disabled={vehicleSaving || savedCar || !userId}
                        ...
                        {savedCar ? (<span className="inline-flex items-center gap-1.5"><CheckCircle size={14} className="text-green-400" />{t.saved}</span>) : vehicleSaving ? t.saving : t.save}
                      </button>
                      <button onClick={() => { setShowVehicleForm(false); setPlate('') }} className="text-xs text-gray-500 px-3">{t.cancel}</button>
                    </div>
                  </div>
                ) : (
          ...
          <button onClick={() => { lightTap(); handleDeleteAccount() }} className="w-full flex items-center justify-center gap-2 px-[18px] py-3.5 text-gray-400">
            <Trash2 size={15} strokeWidth={1.6} />
            <span className="text-[13px] font-medium">{t.deleteAccount}</span>
          </button>
        </div>
